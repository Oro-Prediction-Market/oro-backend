...
export class AmlService {
  ...
  async getAlert(id: string): Promise<AmlAlert> {
    const alert = await this.alertRepo.findOne({ where: { id }, relations: ["user"] });
    if (!alert) throw new NotFoundException(`AML alert ${id} not found`);
    return alert;
  }

  async resolveAlert(id: string, adminId: string, resolution: string): Promise<AmlAlert> {
  ...
  async getReportWithAlerts(reportId: string): Promise<{ report: AmlReport; alerts: AmlAlert[] }> {
