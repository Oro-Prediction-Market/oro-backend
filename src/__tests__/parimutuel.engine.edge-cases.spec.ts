...
function makeEm(positions: any[]) {
  ...
  // Chainable query builder covering the bulk paths the engine uses:
  //   balances: createQueryBuilder("t").select().addSelect().where().groupBy().getRawMany()
  //   inserts:  createQueryBuilder().insert().into().values(rows).execute()
  //   updates:  createQueryBuilder().update().set().where().execute()
  const makeQb = () => {
    const qb: any = {
      select: () => qb,
      addSelect: () => qb,
      ...
      set: () => qb,
      values: (rows: any) => {
        for (const r of Array.isArray(rows) ? rows : [rows]) saved.push([null, r]);
        return qb;
      },
      // Balance aggregation: return a mocked ledger balance of 100 per bettor.
  ...
  return {
    // Position queries have no `select`; User look-ups (bonus fields / telegramId)
    // pass a `select`, so return one user row per unique bettor for those.
    find: jest.fn().mockImplementation((_Entity: any, opts: any) => {
      if (opts?.select) {
        const uids = [...new Set(positions.map((p) => p.userId))];
